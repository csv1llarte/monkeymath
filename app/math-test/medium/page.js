...
function getQuestion() {
  ...
  const answer = op === "×" ? a * b : (a * b) / a;
  return { a, b, op, answer: op === "×" ? a * b : b };
}
    ...
    <main style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minHeight: '100vh', gap: '1.5rem', justifyContent: 'center' }}>
      ...
      {end ? (
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
          <div style={{ fontSize: '1.5rem' }}>{question.op === "×" ? `${question.a} × ${question.b}` : `${question.a * question.b} ÷ ${question.a}`} = ?</div>
          <input
            type="number"
            value={input}
